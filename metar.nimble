...
task md, "Build debug version of metar and python module":
  var output: string
  when hostOS == "macosx":
    output = "bin/mac/debug/metar"
  elif hostOS == "linux":
    output = "bin/linux/debug/metar"
  else:
    output = "bin/debug/metar"

  exec r"nim c --out:$1 metar/metar" % [output]
  exec r"find . -name \*.pyc -delete"
  # The python shared lib must be the same name as the nim module.
  exec r"nim c -d:buildingLib --threads:on --tlsEmulation:off --app:lib --out:$1.so metar/metar " % [output]
